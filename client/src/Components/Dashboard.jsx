...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000s/auth/verify").then((res) => {
      if (res.data.status) {
      } else {
        navigate("/");
      }
      ...
      <div className="flex flex-wrap items-center justify-center w-full max-w-4xl mt-8">
        <div className="flex flex-col flex-grow mt-8 overflow-hidden bg-white rounded-lg shadow-lg">
          ...
          <div className="flex px-10 pb-10 justify-center">
            <button className="flex items-center justify-center w-full h-12 px-6 text-sm uppercase bg-gray-200 rounded-lg hover:bg-gradient-to-r hover:from-blue-500 hover:to-green-500 hover:text-white">
              Join now
            </button>
          </div>
        </div>
